...
const getSyncActionLabel = (value?: 'created' | 'updated', entity?: string) => {
  if (!value || !entity) {
    return '未落地';
  }

  return value === 'created' ? `新建${entity}` : `复用${entity}`;
};

export const ChinaLeadsIndexPage = () => {
  const [selectedJobId, setSelectedJobId] = useState<string | null>(null);
  const [keywords, setKeywords] = useState('跨境电商\ntiktok跨境');
  const [executorBaseUrl, setExecutorBaseUrl] = useState(
    'http://127.0.0.1:4318',
  );
  ...
  const failedCount = collectedItems.filter(
    (item) => item.syncStatus === 'failed',
  ).length;
  const landedCustomersCount = collectedItems.filter(
    (item) => !!item.customerId,
  ).length;
  const landedDealsCount = collectedItems.filter(
    (item) => !!item.dealId,
  ).length;
  const funnelSummary = useMemo(() => {
    ...
    for (const item of importedItems) {
      const label =
        item.dealStageName || item.funnelStageLabel || '未进入销售阶段';
      summary.set(label, (summary.get(label) || 0) + 1);
    }

    return Array.from(summary.entries()).slice(0, 4);
  }, [importedItems]);
      ...
      <div className="flex flex-col gap-4 p-6 w-full overflow-auto">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button
              variant="secondary"
              onClick={handleSync}
              disabled={syncing || !selectedJob?._id}
            >
              导入销售漏斗
            </Button>
            <Button onClick={handleNewJob} disabled={running}>
              New Job
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-md border p-4">
            ...
            <div className="mt-2 font-medium">采集抖音评论线索</div>
          </div>
          <div className="rounded-md border p-4">
            <div className="text-sm text-muted-foreground">阶段 2</div>
            <div className="mt-2 font-medium">自动落地到联系人与标签</div>
          </div>
          <div className="rounded-md border p-4">
            <div className="text-sm text-muted-foreground">阶段 3</div>
            <div className="mt-2 font-medium">进入销售商机并持续推进</div>
          </div>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[320px,1fr]">
          <div className="rounded-md border p-4 flex flex-col gap-4">
            ...
            <Textarea
              rows={2}
              value={executorBaseUrl}
              onChange={(event) =>
                setExecutorBaseUrl(event.currentTarget.value)
              }
            />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1.5">
                ...
                <Input
                  value={scrollLoops}
                  onChange={(event) =>
                    setScrollLoops(event.currentTarget.value)
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setSearchTimeoutMs(event.currentTarget.value)
                  }
                />
              </div>
              <div className="grid gap-1.5">
                ...
                <Input
                  value={filterTimeoutMs}
                  onChange={(event) =>
                    setFilterTimeoutMs(event.currentTarget.value)
                  }
                />
              </div>
            </div>
          ...
          <div className="rounded-md border p-4 flex flex-col gap-4">
            ...
            <div className="grid gap-2">
              ...
              {!!funnelSummary.length && (
                <div className="grid gap-3 md:grid-cols-4">
                  {funnelSummary.map(([label, count]) => (
                    <div
                      key={label}
                    ...
                    {collectedItems.slice(0, 20).map((item) => (
                      <div key={item._id} className="rounded border p-3">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <div className="font-medium">
                              {item.commentNickname || '匿名用户'}
                            </div>
                            <div className="text-sm text-muted-foreground mt-1">
                              {item.keyword || item.channel} ·{' '}
                              {item.ipLocation || '未知地区'}
                            </div>
                          </div>
                        ...
                        <div className="mt-3 grid gap-1 text-sm text-muted-foreground">
                          <div>
                            发布时间: {formatDateTime(item.sourceCreatedAt)}
                          </div>
                    ...
                    {importedItems.slice(0, 20).map((item) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <div className="font-medium">
                              {item.commentNickname || '匿名线索'}
                            </div>
                            <div className="text-sm text-muted-foreground mt-1">
                              来源抖音 · {item.keyword || item.channel} ·{' '}
                              {item.funnelStageLabel || '待触达'}
                            </div>
                          </div>
                          <div className="text-right text-xs text-muted-foreground">
                            <div>{item.syncStatus}</div>
                        ...
                        <div className="mt-3 grid gap-1 text-sm">
                          <div>联系人: {item.customerId || '待生成'}</div>
                          <div>商机: {item.dealId || '待生成'}</div>
                          <div>负责人: {item.customerOwnerId || '未分配'}</div>
                          <div>标签数: {item.customerTagIds?.length || 0}</div>
                          <div>
                            销售阶段:{' '}
                            {item.dealStageName ||
                              item.funnelStageLabel ||
                              '未进入销售阶段'}
                          </div>
                          <div>
                            {getSyncActionLabel(item.dealSyncAction, '商机')}
                          </div>
                          {item.syncError && (
